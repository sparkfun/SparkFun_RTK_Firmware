//These are all the low frequency tasks that are run via the xTask scheduler
...
//void updateDisplayTask()
{
  while (true)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(500)); //Update the display every 500ms
  }
}
...
void checkUbloxTask(void *e)
{
  while (true)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(100)); //Check for new u-blox I2C data every 100ms
    //Serial.printf("GPS time to update: %d\n", millis() - startTime);
  }
}

//Control BT status LED according to bluetoothState
void updateBTLEDTask(void *e)
{
  while (1)
  {
...
void checkBatteryTask(void *e)
{
  while (true)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(2000)); //Check batt levels every 2s
  }
}
