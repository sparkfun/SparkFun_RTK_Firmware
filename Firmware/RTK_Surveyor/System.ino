...
bool configureUbloxModule()
{
  ...
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1OUTPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1OUTPROT_NMEA, 1);
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1OUTPROT_RTCM3X, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1INPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1INPROT_NMEA, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1INPROT_RTCM3X, 1);
  if (zedModuleType == PLATFORM_F9P && zedFirmwareVersionInt >= 120)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1INPROT_SPARTN, 0);

  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART1_BAUDRATE, settings.dataPortBaud); //Defaults to 230400 to maximize message output support
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2_BAUDRATE, settings.radioPortBaud); //Defaults to 57600 to match SiK telemetry radio firmware default

  //Disable SPI port - This is just to remove some overhead by ZED
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIOUTPROT_UBX, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIOUTPROT_NMEA, 0);
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIOUTPROT_RTCM3X, 0);

  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIINPROT_UBX, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIINPROT_NMEA, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIINPROT_RTCM3X, 0);
  if (zedModuleType == PLATFORM_F9P && zedFirmwareVersionInt >= 120)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_SPIINPROT_SPARTN, 0);

  //Set the UART2 to only do RTCM (in case this device goes into base mode)
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2OUTPROT_UBX, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2OUTPROT_NMEA, 0);
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2OUTPROT_RTCM3X, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2INPROT_UBX, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2INPROT_NMEA, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2INPROT_RTCM3X, 1);
  if (zedModuleType == PLATFORM_F9P && zedFirmwareVersionInt >= 120)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_UART2INPROT_SPARTN, 0);

  //We don't want NMEA over I2C, but we will want to deliver RTCM, and UBX+RTCM is not an option
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2COUTPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2COUTPROT_NMEA, 1);
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2COUTPROT_RTCM3X, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2CINPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2CINPROT_NMEA, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2CINPROT_RTCM3X, 1);
  if (zedModuleType == PLATFORM_F9P && zedFirmwareVersionInt >= 120)
  {
    if (productVariant == RTK_FACET_LBAND)
      response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2CINPROT_SPARTN, 1); //We push NEO-D9S correction data (SPARTN) to ZED-F9P over the I2C interface
    else
      response &= i2cGNSS.addCfgValset(UBLOX_CFG_I2CINPROT_SPARTN, 0);
  }
  ...
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBOUTPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBOUTPROT_NMEA, 1);
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBOUTPROT_RTCM3X, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBINPROT_UBX, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBINPROT_NMEA, 1);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBINPROT_RTCM3X, 1);
  if (zedModuleType == PLATFORM_F9P && zedFirmwareVersionInt >= 120)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_USBINPROT_SPARTN, 0);
  ...
  response &= i2cGNSS.newCfgValset();
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GGA_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSA_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSV_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_RMC_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GST_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GLL_I2C, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_VTG_I2C, 0);

  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GGA_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSA_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSV_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_RMC_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GST_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GLL_UART2, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_VTG_UART2, 0);

  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GGA_SPI, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSA_SPI, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GSV_SPI, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_RMC_SPI, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GST_SPI, 0);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_GLL_SPI, 0);
  response &= i2cGNSS.sendCfgValset(UBLOX_CFG_MSGOUT_NMEA_ID_VTG_SPI, 0);
  response &= i2cGNSS.sendCfgValset();
...
bool setMessages()
{
  ...
  for (int x = 0 ; x < 36 ; x++)
  {
    if (settings.ubxMessages[x].supported & zedModuleType)
      response &= i2cGNSS.addCfgValset(settings.ubxMessages[x].msgConfigKey, settings.ubxMessages[x].msgRate);
  }
  response &= i2cGNSS.sendCfgValset();
...
bool setMessagesUSB()
{
  ...
  {
    if (settings.ubxMessages[x].supported & zedModuleType)
      response &= i2cGNSS.addCfgValset(settings.ubxMessages[x].msgConfigKey + 2, settings.ubxMessages[x].msgRate);
  }
  response &= i2cGNSS.sendCfgValset();
...
bool setConstellations(bool sendCompleteBatch)
{
  ...
  response &= i2cGNSS.addCfgValset(settings.ubxConstellations[0].configKey, settings.ubxConstellations[0].enabled); //GPS
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GPS_L1CA_ENA, settings.ubxConstellations[0].enabled);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GPS_L2C_ENA, settings.ubxConstellations[0].enabled);
  ...
  {
    response &= i2cGNSS.addCfgValset(settings.ubxConstellations[1].configKey, settings.ubxConstellations[1].enabled); //SBAS
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_SBAS_L1CA_ENA, settings.ubxConstellations[1].enabled);
  }

  response &= i2cGNSS.addCfgValset(settings.ubxConstellations[2].configKey, settings.ubxConstellations[2].enabled); //GAL
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GAL_E1_ENA, settings.ubxConstellations[2].enabled);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GAL_E5B_ENA, settings.ubxConstellations[2].enabled);

  response &= i2cGNSS.addCfgValset(settings.ubxConstellations[3].configKey, settings.ubxConstellations[3].enabled); //BDS
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_BDS_B1_ENA, settings.ubxConstellations[3].enabled);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_BDS_B2_ENA, settings.ubxConstellations[3].enabled);

  response &= i2cGNSS.addCfgValset(settings.ubxConstellations[4].configKey, settings.ubxConstellations[4].enabled); //QZSS
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_QZSS_L1CA_ENA, settings.ubxConstellations[4].enabled);

  //UBLOX_CFG_SIGNAL_QZSS_L1S_ENA not supported on F9R in v1.21 and below
  if (zedModuleType == PLATFORM_F9P)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_QZSS_L1S_ENA, settings.ubxConstellations[4].enabled);
  else if (zedModuleType == PLATFORM_F9R && zedFirmwareVersionInt > 121)
    response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_QZSS_L1S_ENA, settings.ubxConstellations[4].enabled);

  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_QZSS_L2C_ENA, settings.ubxConstellations[4].enabled);

  response &= i2cGNSS.addCfgValset(settings.ubxConstellations[5].configKey, settings.ubxConstellations[5].enabled); //GLO
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GLO_L1_ENA, settings.ubxConstellations[5].enabled);
  response &= i2cGNSS.addCfgValset(UBLOX_CFG_SIGNAL_GLO_L2_ENA, settings.ubxConstellations[5].enabled);
